...
export default function MatchPopup({
  ...
  const handleSimulate = () => {
    ...
    // Auto-generate events if all required halves are simulated and we have enough players
    const isRegularDone = newSimulated.has("h1") && newSimulated.has("h2");
    const isExtraDone = showExtraTime ? newSimulated.has("et1") && newSimulated.has("et2") : true;
    
    if (isRegularDone && isExtraDone && canLiveSimulate && homeTeam && awayTeam) {
      const stats = generateMatchStats(homeRate, awayRate, totalHome + (isET ? h : 0), totalAway + (isET ? a : 0));
      setMatchStats(stats);
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onCancel}>
      ...
      >
                    ...
                    <TooltipTrigger asChild>
                      <div className="w-full max-w-xs">
                        <button
                          ...
                          className="w-full py-3 rounded-xl bg-accent text-accent-foreground font-display font-bold text-sm hover:bg-accent/80 transition-colors flex items-center justify-center gap-2"
                        >
        ...
        {showBottomPanel && (displayStats || hasEvents) && (
          <div className="border-t border-border">
            <Tabs value={bottomTab} onValueChange={(v) => setBottomTab(v as "stats" | "events")}>
              ...
              <TabsContent value="stats" className="px-6 py-6 mt-0">
                {displayStats ? (
                  <div className="grid grid-cols-2 gap-x-12 gap-y-6">
                    ...
                    <StatRow label="Chutes no Gol" homeValue={displayStats.homeStats.shotsOnTarget} awayValue={displayStats.awayStats.shotsOnTarget} />
                  ...
                  <div className="py-12 text-center text-muted-foreground text-xs italic">
                    Estatísticas serão geradas ao finalizar ou simular minuto a minuto.
                  </div>
                )}
              </TabsContent>

              <TabsContent value="events" className="px-6 py-4 mt-0">
        ...
        {bottomStandings.length > 0 && (
          <div className="bg-secondary/30 border-t border-border px-6 py-4">
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-3">
              {standingsTitle} (Ao Vivo)
            </p>
            <div className="space-y-2">
              {bottomStandings.map((s) => (
                <div key={s.teamId} className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-muted-foreground w-4">{s.position}º</span>
                    <span className="font-medium text-foreground">{s.teamName}</span>
                  </div>
                  <div className="flex items-center gap-4 font-mono">
                    <span className="text-muted-foreground">
                      {s.played}j {s.won}/{s.drawn}/{s.lost}
                    </span>
                    <span className="text-primary font-bold w-4 text-right">{s.points}pts</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Actions */}
        <div className="bg-secondary/50 border-t border-border px-6 py-4 flex items-center justify-between gap-3">
          <button
            onClick={onCancel}
            className="px-4 py-2 rounded-lg text-sm font-bold text-muted-foreground hover:text-foreground transition-colors"
          >
            Cancelar
          </button>
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-6 py-2 rounded-lg bg-primary text-primary-foreground font-display font-bold text-sm hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20"
            >
              {isKnockout && requiresDeciderAfterRegular ? "Ir para Decisão" : "Salvar Resultado"}
            </button>
          </div>
        </div>
      </div>
