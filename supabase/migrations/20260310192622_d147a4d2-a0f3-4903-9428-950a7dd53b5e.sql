...
-- =====================================================
-- FIX 1: Replace broken token verification policy with RPC function
-- =====================================================

-- Drop the broken token policy that uses share_token IS NOT NULL
DROP POLICY IF EXISTS "published_select_by_token" ON public.published_tournaments;

-- Convert owner SELECT policies from RESTRICTIVE to PERMISSIVE so the app works
...
CREATE POLICY "tournaments_select_published"
  ...
  TO public
  USING (EXISTS (
    SELECT 1 FROM published_tournaments pt
...
-- Convert write policies to PERMISSIVE
DROP POLICY IF EXISTS "published_insert_auth" ON public.published_tournaments;
CREATE POLICY "published_insert_auth"
  ON public.published_tournaments FOR INSERT
  TO authenticated
  WITH CHECK ((auth.uid())::text = user_id);

DROP POLICY IF EXISTS "published_update_owner" ON public.published_tournaments;
CREATE POLICY "published_update_owner"
  ON public.published_tournaments FOR UPDATE
  TO authenticated
  USING ((auth.uid())::text = user_id);

DROP POLICY IF EXISTS "published_delete_owner" ON public.published_tournaments;
CREATE POLICY "published_delete_owner"
  ON public.published_tournaments FOR DELETE
  TO authenticated
  USING ((auth.uid())::text = user_id);

-- tournaments write policies
...
CREATE POLICY "tfolders_select_owner"
  ON public.tournament_folders FOR SELECT
  TO authenticated
  USING ((auth.uid())::text = user_id);

DROP POLICY IF EXISTS "tfolders_insert_auth" ON public.tournament_folders;
CREATE POLICY "tfolders_insert_auth"
  ON public.tournament_folders FOR INSERT
  TO authenticated
  WITH CHECK ((auth.uid())::text = user_id);

DROP POLICY IF EXISTS "tfolders_update_owner" ON public.tournament_folders;
CREATE POLICY "tfolders_update_owner"
  ON public.tournament_folders FOR UPDATE
  TO authenticated
  USING ((auth.uid())::text = user_id);

DROP POLICY IF EXISTS "tfolders_delete_owner" ON public.tournament_folders;
CREATE POLICY "tfolders_delete_owner"
  ON public.tournament_folders FOR DELETE
  TO authenticated
  USING ((auth.uid())::text = user_id);

-- team_histories
DROP POLICY IF EXISTS "histories_select_owner" ON public.team_histories;
CREATE POLICY "histories_select_owner"
  ON public.team_histories FOR SELECT
  TO authenticated
  USING ((auth.uid())::text = user_id);

DROP POLICY IF EXISTS "histories_insert_auth" ON public.team_histories;
CREATE POLICY "histories_insert_auth"
  ON public.team_histories FOR INSERT
  TO authenticated
  WITH CHECK ((auth.uid())::text = user_id);

DROP POLICY IF EXISTS "histories_update_owner" ON public.team_histories;
CREATE POLICY "histories_update_owner"
  ON public.team_histories FOR UPDATE
  TO authenticated
  USING ((auth.uid())::text = user_id);

DROP POLICY IF EXISTS "histories_delete_owner" ON public.team_histories;
CREATE POLICY "histories_delete_owner"
  ON public.team_histories FOR DELETE
  TO authenticated
  USING ((auth.uid())::text = user_id);
...
CREATE POLICY "collaborators_insert_auth"
  ON public.tournament_collaborators FOR INSERT
  TO authenticated
  WITH CHECK (EXISTS (
    SELECT 1 FROM published_tournaments pt
    WHERE pt.id = published_tournament_id
    AND pt.user_id = (auth.uid())::text
  ));
...
-- =====================================================
-- Create RPC function for secure token-based lookup
-- =====================================================
CREATE OR REPLACE FUNCTION public.get_tournament_by_share_token(p_token text)
...
AS $$
  ...
  INNER JOIN public.published_tournaments pt ON pt.tournament_id = t.id
  WHERE pt.share_token = p_token
  LIMIT 1;
$$;

-- =====================================================
-- FIX 2: Restrict storage INSERT to owned paths only
-- =====================================================
