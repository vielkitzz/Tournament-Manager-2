...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: solaraPlayers, error: fetchError } = await solaraClient
      .from("players")
      .select("*")
      .eq("club_id", solarahub_club_id); // <--- MUDE DE "team_id" PARA "club_id"

    if (fetchError) throw fetchError;

    if (!solaraPlayers || solaraPlayers.length === 0) {
      return new Response(
        JSON.stringify({ success: true, message: "Nenhum jogador encontrado no SolaraHub", players: [] }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
